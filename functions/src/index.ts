...
async function generateEmbedding(text: string,
  taskType = "SEMANTIC_SIMILARITY"): Promise<number[]> {
  try {
    logger.info(`Generando embedding para: ${text.substring(0, 50)}... ` +
      `con tipo: ${taskType}`);

    const app = getApp();
    const projectId = process.env.GCLOUD_PROJECT || app.options.projectId;
    ...
    const predictionObj = predictionData as any;

    if (!predictionObj || !predictionObj.embeddings ||
        !predictionObj.embeddings.values) {
 ...
 * @return {number} Similitud (valor entre -1 y 1)
 */
