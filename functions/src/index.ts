...
async function generateEmbedding(text: string): Promise<number[]> {
  try {
    ...
    logger.info(
      `Generando embedding para: ${text.substring(0, 50)}...`
    );
...
/**
 * Cloud Function: Generar embedding cuando se actualiza un perfil profesional
 * Se ejecuta automáticamente cuando se escribe en /profesionales
 */
export const onProfessionalWritten = onDocumentWritten(
  ...
  async (event) => {
    ...
    if (!snapshot.after.exists || !after) {
      return;
    }

    // Si es una actualización, verificar si cambiaron los campos relevantes
    if (snapshot.before.exists && before) {
      const oldText = `${before.rubro_principal || ""} ` +
        `${before.descripcion || ""}`;
      const newText = `${after.rubro_principal || ""} ` +
        `${after.descripcion || ""}`;

      // Si el texto relevante no cambió, y ya tiene embedding, no regenerar
      if (oldText === newText && after.embedding) {
        return;
      }
    }

    const profileText = `${after.rubro_principal || ""} ` +
      `${after.descripcion || ""}`;

    // Evitar generar embedding para perfiles vacíos
    if (!profileText.trim()) {
      return;
    }

    logger.info(`Generando embedding para profesional: ${snapshot.after.id}`);
    ...
    try {
      ...
      logger.info(
        `Embedding actualizado para profesional ${snapshot.after.id}`
      );
    } catch (error) {
      logger.error(
        `Error generando embedding para profesional ${snapshot.after.id}:`,
        error
      );
    }
  }
...
export const findProfessionals = onCall(async (request) => {
  ...
  try {
    ...
    snapshot.forEach((doc) => {
      ...
      // Generar texto del perfil del profesional (para fallback)
      const profileText = `${data.rubro_principal || ""} ` +
        `${data.descripcion || ""}`;

      let similarity = 0;

      // Si tiene embedding, calcular similitud real
      if (data.embedding && Array.isArray(data.embedding)) {
        try {
          similarity = cosineSimilarity(descEmbedding, data.embedding);
        } catch (e) {
          logger.warn(`Error calculando similitud para ${doc.id}`, e);
        }
      } else {
        // Fallback: matching simple o valor por defecto
        // Si coincide el rubro, damos un puntaje base
        similarity = profileText.length > 0 ? 0.1 : 0;
      }
