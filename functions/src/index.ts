 ...
 * @return {number} Similitud de coseno
 */
...
export const semanticSearch = onCall(async (request) => {
  ...
  try {
    ...
    // 2. Utilizar búsqueda vectorial en Firestore
    // Referencia base
    let solicitudesQuery: any = db.collection("solicitudes");

    // Aplicar filtro de zona si existe
    if (zona) {
      solicitudesQuery = solicitudesQuery.where("zona", "==", zona);
    }

    // Ejecutar búsqueda vectorial
    // Nota: Requiere un índice vectorial en el campo 'embedding'
    const vectorQuery = solicitudesQuery.findNearest({
    ...
    snapshot.forEach((doc: any) => {
      ...
      // Recalculamos la similitud para devolverla al cliente
      // Esto es muy rápido porque solo son 'limit' documentos (ej. 10)
      let similarity = 0;
    ...
    return {
      success: true,
      results: results, // Ya están ordenados por findNearest
