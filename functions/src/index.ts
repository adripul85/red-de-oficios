...
export const semanticSearch = onCall(async (request) => {
  ...
  try {
    ...
    let solicitudesQuery: Query = db.collection("solicitudes");

    if (zona) {
      solicitudesQuery = solicitudesQuery.where("zona", "==", zona);
    }

    // Optimization: Use Firestore Vector Search
    // Note: Requires a vector index on 'embedding' field.
    ...
    snapshot.forEach((doc: any) => {
      ...
      // Calculate similarity locally for display purposes
      // findNearest returns documents sorted by similarity
      let similarity = 0;
...
export const findProfessionals = onCall(async (request) => {
  ...
  try {
    ...
    let profQuery: Query = db.collection("profesionales");

    if (zona) {
      profQuery = profQuery.where("zona", "==", zona);
    }
